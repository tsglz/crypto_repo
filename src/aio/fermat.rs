...
pub fn get_max_prime() {
    let mut n = get_num() as u64; // 要测试的数
    let k = 10; // 测试的迭代次数

    // 执行Fermat素性测试
    let (mut is_prime, _) = fermat_primality_test(n, k);
    let mut final_n = 2;

    while !is_prime {
        n -= 1;
        (is_prime, _) = fermat_primality_test(n, k);
    }
